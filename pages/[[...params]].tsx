...
import { getSlugAndTimePeriodFromParams } from '../lib/get-static-params';
...
export const getStaticProps: GetStaticProps = async ({ params: { params } }) => {
  const { hostSlug, timePeriod } = getSlugAndTimePeriodFromParams(params);
  ...
  const collectives = data.accounts.nodes.map(collective => {
    const stats = getTotalStats(collective[timePeriod]);
...
export async function getStaticPaths() {
  ...
  const timePeriodSlug = ['PAST_YEAR', 'PAST_QUARTER'];
  return {
    paths: [
      { params: { params: [] } },
      ...timePeriodSlug.map(timePeriod => ({
        params: { params: [timePeriod] },
      })),
      ...hostSlugs.map(slug => ({
        params: {
          params: [slug],
      ...
      ...hostSlugs.map(slug => ({
        params: {
          params: [slug, 'PAST_YEAR'],
        },
      })),
      ...hostSlugs.map(slug => ({
        params: {
          params: [slug, 'PAST_QUARTER'],
        },
      })),
    ],
    fallback: false,
